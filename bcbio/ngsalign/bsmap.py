...
def align(fastq_file, pair_file, ref_file, names, align_dir, data):
    assert data["analysis"].lower().startswith("wgbs-seq"), "No compatible alignment."
    config = data["config"]
    sample = dd.get_sample_name(data)
    out_prefix = os.path.join(align_dir, dd.get_lane(data))
    ref_file = dd.get_sam_ref(data)

    out_bam = os.path.join(align_dir, "{0}.bam".format(sample))
    final_bam = os.path.join(align_dir, "{0}.sorted.bam".format(sample))
    if file_exists(final_bam):
        data = dd.set_work_bam(data, final_bam)
        return data
    ...
    cmd = "{bsmap} {num_cores} -w 100 -v 0.07 -m 0 -x 1000 -o {tx_out_bam} -d {ref_file} {fastq_files}"
    if pair_file:
    ...
    final_bam = bam.sort(out_bam, config)
    data = dd.set_work_bam(data, final_bam)
    return data
